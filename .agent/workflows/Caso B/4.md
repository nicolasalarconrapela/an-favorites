---
description: Documentación · Caso B (develop → releases/1.X.0)
---
## Paso anterior
...
- **Si se han generado cambios**, dejar preparado el resultado del paso; cualquier commit posterior sigue requiriendo autorización explícita.

### 7.1 Commit de documentación posterior (opcional)

Objetivo:
Realizar el commit de los archivos de documentación actualizados.

Reglas:
- solo se incluirán los archivos de documentación modificados (`CHANGELOG.md`, `RELEASE_NOTES.md`, `RELEASE_NOTES.es.md`).
- no ejecutar este paso sin autorización explícita del usuario.
- el mensaje de commit sugerido es: `docs: update changelog and release notes for <RELEASE_VERSION>`

- **IMPORTANTE**: Al añadir archivos al git, está estrictamente PROHIBIDO usar `git add .`, `git add -A` o añadir todos los archivos indiscriminadamente.
- Solo se debería permitir añadir: `README.md`, `README.es.md`, `CHANGELOG.md`, `RELEASE_NOTES.md`, `RELEASE_NOTES.es.md` y archivos dentro de `resources/walkthrough/`.

Comandos permitidos (con autorización previa):

```bash
git add README.md README.es.md CHANGELOG.md RELEASE_NOTES.md RELEASE_NOTES.es.md resources/walkthrough/*
```

```bash
git commit -m "docs: update changelog and release notes for <RELEASE_VERSION>"
```

- mostrar cada comando antes de ejecutarlo.
- no pedir confirmación intermedia al usuario durante este workflow.
