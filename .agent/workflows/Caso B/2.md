---
description: DIFF [1/2] - Ejecución de comando · Caso B (develop → releases/1.X.0)
---
## Paso anterior
...
Este paso genera el diff base y calcula la versión objetivo de la release.  
El script detecta automáticamente la versión actual desde `package.json` y calcula la rama destino `releases/1.X+1.0`.

- Este archivo solamente se utilizará para generar el archivo diff y las variables del flujo.
...
- Al finalizar este paso, mostrar al usuario la versión bump calculada (`RELEASE_VERSION`) y la rama destino (`TARGET_BRANCH`) antes de continuar.
- Preguntar al usuario si desea avanzar al siguiente paso.
...
- Si el script falla, detener el workflow inmediatamente.
- El comando `git diff` de este script debe cargar las exclusiones desde `.agent/workflows/diff-excludes.txt`.

## 3.1 Ejecutar script de generación de diff

Antes de ejecutar, verificar únicamente si existe exactamente este archivo:

`.agent/workflows/Caso B/2.sh`
...
`bash ".agent/workflows/Caso B/2.sh"`
...
- cambiar `Caso B/2.sh` por otro nombre similar
...
3. mostrar: versión actual en `package.json`, versión objetivo de release (`RELEASE_VERSION`), rama destino calculada (`TARGET_BRANCH`)
4. si no existe bloqueo real, continuar automáticamente al siguiente paso
